...
function loadFirebaseVenues() {
    db.collection('venues').onSnapshot(
        ...
        (err) => { console.warn('[FindMyNight] Firebase non raggiungibile, uso stime locali.', err.message); }
    );
    ...
    db.collection('sponsoredVenues').where('active', '==', true).onSnapshot(
        ...
        (err) => { console.warn('[FindMyNight] Sponsored venues non disponibili.', err.message); }
    );
}
...
async function loadAndRenderUserReviews() {
    ...
    grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:1.35rem;"><div style="width:30px;height:30px;border:3px solid rgba(168,85,247,0.2);border-top-color:#a855f7;border-radius:50%;animation:spin 0.8s linear infinite;margin:0 auto;"></div></div>`;
    try {
        const snap = await db.collection('reviews').orderBy('createdAt', 'desc').limit(12).get();
        const reviews = snap.docs.map(d => ({ id: d.id, ...(d.data() || {}) }));
        if (!reviews.length) { grid.innerHTML = ''; hint.textContent = 'Ancora nessuna recensione inviata dal sito.'; return; }
        ...
        grid.innerHTML = shown.map((r) => {
            ...
            const when = (() => { const ts = r && r.createdAt; const d = ts && typeof ts.toDate === 'function' ? ts.toDate() : null; return d ? d.toLocaleDateString('it-IT') : ''; })();
            return `<div class="review-card"><div class="review-top"><div class="review-venue">${venueName}</div><div class="review-rating">${rating}</div></div><div class="review-meta">${nick}${when ? ` · ${when}` : ''}</div>${text ? `<div class="review-text">${text}</div>` : `<div class="review-empty">Solo valutazione</div>`}</div>`;
        }).join('');
        hint.textContent = `${shown.length} recensioni (community)`;
    } catch (e) {
        grid.innerHTML = '';
        hint.textContent = `Errore: ${e && e.message ? e.message : e}`;
    }
}
...
async function loadAndRenderGoogleReviews() {
    ...
    for (const c of (clubsData || [])) {
        if (c.googleReviews && Array.isArray(c.googleReviews)) {
            for (const rev of c.googleReviews) allReviews.push({ ...rev, venueName: c.name });
        }
    }
    if (!allReviews.length) { grid.innerHTML = ''; hint.textContent = 'Nessuna recensione trovata tramite API Google.'; return; }
...
function enrichClubsWithFirebase(clubs) {
    clubs.forEach(club => {
        ...
        if (!fb && club.id != null) fb = fbVenueData.get(normalizeName(String(club.id))) || null;
        ...
        if (club.firebaseData && typeof club.firebaseData.affluenzaPct === 'number') {
            ...
            club.crowdText = pct >= 85 ? 'Tutto esaurito 🔴' : pct >= 65 ? 'Affollato 🟠' : pct >= 40 ? 'Vivace 🟡' : 'Tranquillo 🟢';
        } else if (club.firebaseData) {
            const fbInner = club.firebaseData;
            const fbAperto = fbInner.aperto !== false;
            if (fbAperto && !isNightclubAutoClosedNow(club)) { club.crowdText = 'Non impostata'; club.crowdPercent = null; }
        } else {
            club.crowdText = 'Non impostata'; club.crowdPercent = null;
        }
        if (fb && fb.isBar === true) { club.isBar = true; }
        else if (club.isBar == null && !isNightclub(club)) { club.isBar = true; }
    });
}
...
(function injectPreconnects() {
    const preconnectDomains = ['https://maps.googleapis.com', 'https://maps.gstatic.com', 'https://overpass-api.de'];
    ...
    ['https://maps.googleapis.com', 'https://maps.gstatic.com'].forEach((href) => {
        ...
        const link = document.createElement('link');
        link.rel = 'dns-prefetch'; link.href = href;
        head.prepend(link);
    });
})();
...
function markOverpassEndpoint429(endpoint) {
    ...
    const backoff = Math.min(OVERPASS_FAIL_MAX_BACKOFF_MS, OVERPASS_429_BACKOFF_MS * Math.pow(2, Math.min(3, fails - 1)));
...
function markOverpassEndpointFail(endpoint, reason) {
    try {
        const prev = _overpassRateLimit.get(endpoint);
        const fails = prev && Number.isFinite(prev.fails) ? prev.fails + 1 : 1;
        const backoff = Math.min(OVERPASS_FAIL_MAX_BACKOFF_MS, OVERPASS_FAIL_BASE_BACKOFF_MS * Math.pow(2, Math.min(3, fails - 1)));
        _overpassRateLimit.set(endpoint, { until: Date.now() + backoff, fails });
...
const ITALY_DISCOVERY_CENTERS = [
    { label: 'Milano', lat: 45.4642, lng: 9.1900 }, { label: 'Torino', lat: 45.0703, lng: 7.6869 },
    { label: 'Genova', lat: 44.4056, lng: 8.9463 }, { label: 'Venezia', lat: 45.4408, lng: 12.3155 },
    { label: 'Bologna', lat: 44.4949, lng: 11.3426 }, { label: 'Firenze', lat: 43.7696, lng: 11.2558 },
    { label: 'Roma', lat: 41.9028, lng: 12.4964 }, { label: 'Napoli', lat: 40.8518, lng: 14.2681 },
    { label: 'Bari', lat: 41.1171, lng: 16.8719 }, { label: 'Catania', lat: 37.5079, lng: 15.0830 },
    { label: 'Palermo', lat: 38.1157, lng: 13.3615 }, { label: 'Cagliari', lat: 39.2238, lng: 9.1217 }
];
...
function debounce(fn, waitMs) {
    let t;
    return (...args) => { window.clearTimeout(t); t = window.setTimeout(() => fn(...args), waitMs); };
}
...
function osmVenueDisplayName(tags) {
    ...
    const pick = (...keys) => { for (const k of keys) { const v = (tags[k] ?? '').toString().trim(); if (v) return v; } return ''; };
    ...
    if (city && base !== 'Locale') {
        const bN = normalizeText(base); const cN = normalizeText(city);
        if (cN && !bN.includes(cN)) base = `${base} di ${city}`;
    }
    return base;
}
...
function osmVenueAddressFromTags(tags) {
    ...
    const full = t('addr:full'); if (full) return full;
    const street = [t('addr:street'), t('addr:housenumber')].filter(Boolean).join(' ').trim();
    const road = street || t('contact:street');
    ...
    const pc = t('addr:postcode'); const prov = t('addr:province');
    ...
    if (road) return road; if (tail) return tail;
    return place || '';
}
...
function buildGoogleDirectionsLink(lat, lng, name) {
    return `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(name ? `${name}` : `${lat},${lng}`)}`;
}

function buildAppleMapsLink(lat, lng, name) {
    return `https://maps.apple.com/?q=${encodeURIComponent(name ? `${name}` : `${lat},${lng}`)}&ll=${lat},${lng}`;
}
...
function closeMapsChooserModal() {
    ...
    if (mapsChooserEscapeHandler) { document.removeEventListener('keydown', mapsChooserEscapeHandler); mapsChooserEscapeHandler = null; }
}

function openInMapsChooser(lat, lng, name) {
    mapsChooserState.lat = lat; mapsChooserState.lng = lng; mapsChooserState.name = name || 'Destinazione';
    const modal = document.getElementById('mapsChooserModal');
    const venueEl = document.getElementById('mapsChooserVenue');
    if (!modal) { window.open(buildGoogleDirectionsLink(lat, lng, mapsChooserState.name), '_blank', 'noopener,noreferrer'); return; }
    ...
    mapsChooserEscapeHandler = (ev) => { if (ev.key === 'Escape') { ev.preventDefault(); closeMapsChooserModal(); } };
    document.addEventListener('keydown', mapsChooserEscapeHandler);
    const gBtn = document.getElementById('mapsChooserGoogle'); if (gBtn) gBtn.focus();
}
...
function wireMapsChooserModal() {
    ...
    if (googleBtn) googleBtn.addEventListener('click', () => { const { lat, lng, name } = mapsChooserState; if (!Number.isFinite(lat) || !Number.isFinite(lng)) return; openAndClose(buildGoogleDirectionsLink(lat, lng, name)); });
    if (appleBtn) appleBtn.addEventListener('click', () => { const { lat, lng, name } = mapsChooserState; if (!Number.isFinite(lat) || !Number.isFinite(lng)) return; openAndClose(buildAppleMapsLink(lat, lng, name)); });
    [backdrop, closeBtn, cancelBtn].forEach((el) => { if (el) el.addEventListener('click', () => closeMapsChooserModal()); });
}

function getSearchCenter() {
    if (manualCenter && userLocation) { const d = kmBetween(manualCenter, userLocation); if (d <= SAME_AREA_GPS_VS_PLACE_KM) return userLocation; }
    return manualCenter || userLocation || discoveryCenter || DEFAULT_CENTER;
}

function getVenueListReferenceCenter() {
    if (manualCenter && userLocation) { const d = kmBetween(manualCenter, userLocation); if (d > SAME_AREA_GPS_VS_PLACE_KM) return manualCenter; }
...
function getOverpassCache(center, radiusMeters) {
    ...
    if (mem) { if (Date.now() - mem.ts <= OVERPASS_CACHE_TTL_MS) return mem.data; overpassMemCache.delete(key); }
    try {
...
function setOverpassCache(center, radiusMeters, data) {
    ...
    try { localStorage.setItem(key, JSON.stringify({ ts: Date.now(), data })); } catch { /* quota exceeded */ }
}
...
function getOverpassCacheFiltered(center, radiusMeters) {
    ...
    const tryRadii = [radiusMeters, 28000, 30000, 35000, 40000, 22000, 20000, 15000, 12000];
    const seen = new Set();
...
async function fetchClubsOverpassAnyEndpoint(center, radiusMeters, signal) {
    ...
    const promise = (async () => {
        ...
        if (Array.isArray(results) && results.length) setOverpassCache(center, radiusMeters, results);
        return results || [];
    })();
    OVERPASS_INFLIGHT.set(inflightKey, promise);
    try { return await promise; } finally { OVERPASS_INFLIGHT.delete(inflightKey); }
}
...
function isOsmVenueLikelyClosed(tags) {
    ...
    if (g('demolished') === 'yes' || g('ruined') === 'yes' || g('removed') === 'yes') return true;
    if (tags['disused:amenity'] || tags['disused:leisure'] || tags['disused:shop']) return true;
    if (tags['was:amenity'] || tags['was:leisure'] || tags['end_date']) return true;
...
function mapOsmElementsToClubs(elements, center) {
    ...
    return elementsFiltered.map((el) => {
        ...
        return { id: `${el.type}/${el.id}`, name, address, lat, lng, distanceKm, ratingText: '—', starsText: '—', crowdText: '—', crowdPercent: 0, ageText: '—', source: 'OSM', osmAmenity: tags.amenity || '', osmLeisure: tags.leisure || '', osmClub: tags.club || '', isBar };
    }).filter(Boolean).sort((a, b) => a.distanceKm - b.distanceKm).slice(0, OVERPASS_MAX_RESULTS);
}

async function fetchNightclubsFromOverpass({ endpoint, center, radiusMeters, signal, timeoutMs = 9000 }) {
    if (isOverpassEndpointThrottled(endpoint)) throw new Error(`Overpass endpoint in backoff: ${endpoint}`);
    const lat = Number(center.lat); const lng = Number(center.lng);
    const r = Math.max(1000, Math.min(50000, Math.round(radiusMeters)));
    const query = `[out:json][timeout:10];(node["amenity"="nightclub"](around:${r},${lat},${lng});way["amenity"="nightclub"](around:${r},${lat},${lng});relation["amenity"="nightclub"](around:${r},${lat},${lng});node["club"="nightclub"](around:${r},${lat},${lng});way["club"="nightclub"](around:${r},${lat},${lng});node["leisure"="nightclub"](around:${r},${lat},${lng});node["leisure"="dancing"](around:${r},${lat},${lng});node["amenity"="music_venue"](around:${r},${lat},${lng});node["amenity"="bar"]["club"="nightclub"](around:${r},${lat},${lng}););out center tags;`;
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), timeoutMs);
    let res;
    try {
        res = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }, body: new URLSearchParams({ data: query }), signal: controller.signal });
    } catch (e) {
        const msg = e && e.name === 'AbortError' ? 'timeout' : (e && e.message ? e.message : String(e));
        markOverpassEndpointFail(endpoint, msg); throw e;
    } finally { clearTimeout(timer); }
    if (res.status === 429) { markOverpassEndpoint429(endpoint); throw new Error(`Overpass 429`); }
    if (!res.ok) { if (res.status >= 500 || res.status === 0) markOverpassEndpointFail(endpoint, `http_${res.status}`); throw new Error(`Overpass HTTP ${res.status}`); }
...
function formatProssimoEventoDisplay(raw) {
    ...
    if (typeof raw.toDate === 'function') { try { const d = raw.toDate(); if (Number.isNaN(d.getTime())) return ''; return d.toLocaleDateString('it-IT', { day: 'numeric', month: 'long', year: 'numeric' }); } catch { return ''; } }
    if (typeof raw === 'number' && Number.isFinite(raw)) { const d = new Date(raw); if (Number.isNaN(d.getTime())) return ''; return d.toLocaleDateString('it-IT', { day: 'numeric', month: 'long', year: 'numeric' }); }
...
function deriveVibeDetails(club) {
    ...
    if (fb) {
        ...
        if (autoClosed) { crowdPct = 0; } else if (!fbAperto) { crowdPct = 0; } else if (hasAff) { crowdPct = Math.round(fb.affluenzaPct); } else { crowdPct = null; affluenzaNonImpostata = true; }
        ...
        if (hasIngressPartner) { entryEuro = fb.ingresso; entryNonImpostato = false; } else if (publishedEntryOk) { entryEuro = published.entryEuro; entryNonImpostato = false; }
        ...
        return {
            crowdPct, affluenzaNonImpostata, entryEuro, entryNonImpostato, drinkEuro, drinkNonImpostato,
            ageRange: strOk(fb.ageRange) ? fb.ageRange.trim() : null, ageNonImpostato: !strOk(fb.ageRange),
            music: strOk(fb.musica) ? fb.musica.trim() : null, musicNonImpostato: !strOk(fb.musica),
            peak: fb.picco != null && String(fb.picco).trim() ? String(fb.picco).trim() : null,
            piccoNonImpostato: !fb.picco || !String(fb.picco).trim(),
            eventoNome: fb.eventoNome || null, prossimoEventoLabel: formatProssimoEventoDisplay(rawProssimo),
            aperto: autoClosed ? false : fbAperto, isBar: fb.isBar === true, isLive: fb.isBar !== true
        };
    }
    let crowdPct = null; let affluenzaNonImpostataFb = true;
    if (autoClosed) { crowdPct = 0; affluenzaNonImpostataFb = false; }
    else if (typeof club.crowdPercent === 'number' && club.crowdPercent > 0 && club.crowdPercent <= 100) { crowdPct = Math.round(club.crowdPercent); affluenzaNonImpostataFb = false; }
    let entryEuro = null; let entryNonImpostato = true;
...
function showClubDetails(club) {
    ...
    musEl.textContent = vibe.musicNonImpostato ? (vibe.eventoNome ? `Serata: ${vibe.eventoNome}` : 'Non impostata') : (vibe.eventoNome ? `${vibe.music} · Serata: ${vibe.eventoNome}` : vibe.music);
    ...
    if (prossimoEl) {
        ...
        prossimoEl.textContent = pe || 'Non impostata'; prossimoEl.style.color = pe ? '' : 'rgba(136, 128, 168, 0.95)';
    }
    let liveBadge = popup.querySelector('.fmn-live-badge');
    if (!liveBadge) {
        liveBadge = document.createElement('span'); liveBadge.className = 'fmn-live-badge';
    ...
    if (!vibe.aperto) { liveBadge.textContent = '● CHIUSO'; liveBadge.style.color = '#fca5a5'; liveBadge.style.background = 'rgba(239,68,68,0.12)'; liveBadge.style.border = '0.5px solid rgba(239,68,68,0.35)'; liveBadge.style.display = 'inline'; }
    else if (vibe.isBar) { liveBadge.textContent = '● APERTO'; liveBadge.style.color = '#cbd5e1'; liveBadge.style.background = 'rgba(148,163,184,0.12)'; liveBadge.style.border = '0.5px solid rgba(148,163,184,0.35)'; liveBadge.style.display = 'inline'; }
    else if (vibe.isLive) { liveBadge.textContent = '● LIVE'; liveBadge.style.color = '#4ade80'; liveBadge.style.background = 'rgba(74,222,128,0.12)'; liveBadge.style.border = '0.5px solid rgba(74,222,128,0.35)'; liveBadge.style.display = 'inline'; }
    else { liveBadge.style.display = 'none'; }
    dirEl.dataset.lat = String(club.lat); dirEl.dataset.lng = String(club.lng); dirEl.dataset.name = String(club.name || 'Locale');
...
function renderMarkers(clubs) { if (!leafletMap) return; clearMarkers(); clubs.forEach(addMarkerForClub); }
...
function geocodeScoreResult(r) {
    const cls = (r.class || '').toLowerCase(); const typ = (r.type || '').toLowerCase();
...
async function geocodeItaly(query) {
    ...
    try {
        ...
        if (raw) {
            ...
            if (o && Number.isFinite(o.lat) && Number.isFinite(o.lng) && o.ts && (Date.now() - o.ts) < (14 * 24 * 60 * 60 * 1000)) {
                ...
                geocodeMemCache.set(key, result); return result;
            }
        }
    } catch { /* ignore */ }
    ...
    const data = await res.json(); if (!data || !data.length) return null;
...
function applySearchFilter(query) {
    ...
    const visible = !q ? [...clubsData] : clubsData.filter(c => normalizeText(c.name).includes(q) || normalizeText(c.address).includes(q));
    renderMarkers(visible); renderSidebar(visible);
}
...
function loadGoogleMapsPlacesIfNeeded() {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (existing) {
            ...
            existing.addEventListener('load', () => { if (window.google && window.google.maps) resolve(window.google); else reject(new Error('Google Maps JS non pronto')); });
        ...
        window[cbName] = () => {
            try { if (window.google && window.google.maps && window.google.maps.places) resolve(window.google); else reject(new Error('Google Maps JS non pronto (callback)')); }
...
async function ensurePlacesLibrary() {
    ...
    if (!_placesLibPromise) {
        _placesLibPromise = g.maps.importLibrary('places').catch((e) => { _placesLibPromise = null; throw e; });
    }
...
function placeDisplayName(p) {
    ...
    if (dn && typeof dn === 'object') { if (typeof dn.text === 'string') return dn.text; if (typeof dn.localizedText === 'string') return dn.localizedText; }
    return '';
}
...
// ─── enrichOneClub: cerca il match Google più vicino per nome+coordinate ──────
async function enrichClubsWithGoogleRatings(clubs) {
    ...
    async function enrichOneClub(club) {
        try {
            ...
            // Cerca su più categorie in parallelo con raggio stretto (200m) per precisione
            const searchTypes = [['night_club'], ['bar'], ['restaurant', 'cafe']];
            const allFound = new Map();

            await Promise.allSettled(searchTypes.map(async (types) => {
                try {
                    const resp = await lib.Place.searchNearby({
                        locationRestriction: { center: biasCenter, radius: 200 },
                        includedTypes: types,
                        maxResultCount: 5,
                        language: 'it',
                        region: 'IT',
                    ...
                    if (resp && Array.isArray(resp.places)) {
                        for (const p of resp.places) { if (p?.id && !allFound.has(p.id)) allFound.set(p.id, p); }
                    }
                } catch { /* ignora categorie non disponibili */ }
            }));

            // Se nessun risultato a 200m, allarga a 500m solo su night_club e bar
            if (!allFound.size) {
                await Promise.allSettled([['night_club'], ['bar']].map(async (types) => {
                    try {
                        const resp = await lib.Place.searchNearby({
            ...
            const places = [...allFound.values()];
            if (!places.length) return stripPending(club, { ratingText: '—', starsText: '—', source: 'OSM' });

            // Scegli il match migliore: prima per nome, poi per distanza
            const scored = places.map((p) => {
                const pn = normalizeText(placeDisplayName(p));
                const ll = placeLatLng(p);
                const distM = ll ? kmBetween(biasCenter, ll) * 1000 : 9999;
                const nameMatch = qn && pn && (pn.includes(qn) || qn.includes(pn)) ? 0 : 1;
                return { p, nameMatch, distM };
            }).sort((a, b) => (a.nameMatch - b.nameMatch) || (a.distM - b.distM));

            const p0 = scored[0]?.p || null;
    ...
    for (let i = 0; i < clubs.length; i += BATCH_SIZE) {
        ...
        if (i + BATCH_SIZE < enrichLimit) await new Promise((r) => setTimeout(r, 80));
    }
...
async function fetchNightclubsFromGooglePlaces({ center, radiusMeters }) {
    ...
    const mergeById = (lists) => {
        ...
        for (const list of lists) {
            ...
            for (const p of arr) { const pid = p && p.id ? String(p.id) : ''; if (!pid || map.has(pid)) continue; map.set(pid, p); }
        }
        return [...map.values()];
    };
    let pooled = [];
    try {
        const [clubsRes, barsRes] = await Promise.all([
            lib.Place.searchNearby({ locationRestriction, includedTypes: ['night_club'], maxResultCount: 20, language: 'it', region: 'IT', fields: baseFields }),
            lib.Place.searchNearby({ locationRestriction, includedTypes: ['bar'], maxResultCount: 20, language: 'it', region: 'IT', fields: baseFields })
        ]);
        pooled = mergeById([clubsRes, barsRes]);
    } catch (e) { console.warn('Place.searchNearby failed:', e); }
    if (!pooled.length) return [];
    return pooled.map((p) => {
        const ll = placeLatLng(p); if (!ll) return null;
        ...
        return { id: `google:${String(p.id || placeDisplayName(p) || 'place')}`, name: placeDisplayName(p) || 'Locale', address: '', lat: ll.lat, lng: ll.lng, distanceKm: kmBetween(center, { lat: ll.lat, lng: ll.lng }), ratingText, starsText, ratingsPending: false, crowdText: '—', crowdPercent: 0, ageText: '—', source: 'Google', googlePlaceTypes: gTypes.slice(), isBar };
...
function addMarkerForClub(club) {
    ...
    const bubbleBg = isNc ? 'linear-gradient(135deg, #38bdf8, #2563eb)' : 'linear-gradient(135deg, #a855f7, #ec4899)';
...
function sanitizePartnerStickerColor(c) {
    ...
    if (/^#([0-9a-fA-F]{3})$/.test(s)) { const h = s.slice(1); return `#${h[0]}${h[0]}${h[1]}${h[1]}${h[2]}${h[2]}`.toLowerCase(); }
    return '#c084fc';
}

function isMapSidebarMobileLayout() { return window.matchMedia('(max-width: 768px)').matches; }

function hideMapFetchLoading() {
    const el = document.getElementById('mapFetchStatus'); if (!el) return;
    el.classList.remove('map-fetch-status--on'); el.innerHTML = ''; el.setAttribute('aria-hidden', 'true');
}
...
function showMapFetchLoading(hasActive) {
    ...
    el.classList.add('map-fetch-status--on'); el.setAttribute('aria-hidden', 'false');
}

let _fmnThirdPartyDeferred = false;
function loadDeferredThirdPartyScripts() {
    if (_fmnThirdPartyDeferred) return; _fmnThirdPartyDeferred = true;
    const defer = (fn) => { if (typeof requestIdleCallback === 'function') requestIdleCallback(fn, { timeout: 4000 }); else setTimeout(fn, 2500); };
    defer(() => {
        const existingGtm = document.querySelector('script[src*="googletagmanager.com/gtm"]');
        if (!existingGtm && window._fmnGtmId) {
            (function (w, d, s, l, i) { w[l] = w[l] || []; w[l].push({ 'gtm.start': new Date().getTime(), event: 'gtm.js' }); var f = d.getElementsByTagName(s)[0], j = d.createElement(s), dl = l !== 'dataLayer' ? '&l=' + l : ''; j.async = true; j.src = 'https://www.googletagmanager.com/gtm.js?id=' + i + dl; f.parentNode.insertBefore(j, f); })(window, document, 'script', 'dataLayer', window._fmnGtmId);
        }
    });
}
...
function renderSidebar(clubs) {
    ...
    sidebar.innerHTML = '';
    if (!clubs.length) {
        sidebar.innerHTML = `<div class="club-card active" style="cursor:default;"><div class="club-card-header"><div class="club-name">Nessun locale trovato</div><div class="club-stars">—</div></div><div class="club-meta">Prova a ricaricare o aumenta il raggio.</div><div class="club-tags"><span class="tag tag-music">Discoteca</span></div></div>`;
        return;
    }
    const center = getSearchCenter();
    const nearbySponsored = sponsoredVenues.filter(sv => sv.lat && sv.lng && sv.radiusKm && kmBetween(center, { lat: sv.lat, lng: sv.lng }) <= sv.radiusKm);
    nearbySponsored.forEach(sv => {
        ...
        card.innerHTML = `<div class="sponsored-label">✦ Suggerito dall'AI</div><div class="club-card-header"><div class="club-name">${escapeHtml(sv.name || 'Locale')}</div><div class="club-stars">—</div></div><div class="club-meta">${dkm} km${sv.address ? ' · ' + escapeHtml(sv.address) : ''}</div><div class="club-tags"><span class="tag tag-ai">Suggerito AI</span>${sv.tagline ? `<span class="tag tag-music">${escapeHtml(sv.tagline)}</span>` : ''}</div>`;
        card.addEventListener('click', () => { setActiveCard(card); if (leafletMap) leafletMap.setView([sv.lat, sv.lng], Math.max(14, leafletMap.getZoom())); if (sv.link) window.open(sv.link, '_blank', 'noopener'); });
    ...
    clubs.forEach((club) => {
        ...
        const crowdLabel = closed ? '<span style="color:#fca5a5;font-weight:600;">Chiuso</span>' : (club.crowdText === 'Non impostata' ? '<span style="color:rgba(136,128,168,0.98);font-weight:500;">Non impostata</span>' : `<span style="color:#c084fc;font-weight:500;">${club.crowdText || '—'}</span>`);
        const fb = club.firebaseData || null;
        ...
        const badgeHtml = closed ? ' <span style="font-size:0.62rem;font-weight:700;color:#fca5a5;background:rgba(239,68,68,0.12);border:0.5px solid rgba(239,68,68,0.3);border-radius:999px;padding:0.1rem 0.45rem;margin-left:0.3rem;letter-spacing:0.06em;">CHIUSO</span>' : (fbIsBar && fbAperto ? ' <span style="font-size:0.62rem;font-weight:700;color:#cbd5e1;background:rgba(148,163,184,0.12);border:0.5px solid rgba(148,163,184,0.3);border-radius:999px;padding:0.1rem 0.45rem;margin-left:0.3rem;letter-spacing:0.06em;">APERTO</span>' : (fbAperto ? ' <span style="font-size:0.62rem;font-weight:700;color:#4ade80;background:rgba(74,222,128,0.12);border:0.5px solid rgba(74,222,128,0.3);border-radius:999px;padding:0.1rem 0.45rem;margin-left:0.3rem;letter-spacing:0.06em;">LIVE</span>' : ''));
        ...
        const addPartnerCustomSticker = (text, colorHex) => { const col = sanitizePartnerStickerColor(colorHex); const safe = escapeHtml(String(text || '').slice(0, 28)); stickerHtmlParts.push(`<span class="tag" style="border:1px solid ${col};color:${col};background:rgba(255,255,255,0.04);">${safe}</span>`); };
        if (showMapStickers) {
            stickersRaw.slice(0, 4).forEach((item) => {
                if (item && typeof item === 'object' && !Array.isArray(item) && String(item.type) === 'custom') { const tx = String(item.text || '').trim().slice(0, 28); if (tx) addPartnerCustomSticker(tx, item.color); return; }
            ...
            if (!stickerHtmlParts.length) { club.isBar ? addSticker('Bar', 'tag-free') : addSticker(club.seedTipo || 'Discoteca', 'tag-music'); if (club.seedIngresso === 0) addSticker('Ingresso gratis', 'tag-free'); }
        }
        ...
        const crowdBlock = showMapAff ? `<div class="crowd-bar"><div class="crowd-fill" style="width:${crowdPct}%"></div></div><div class="crowd-label"><span>Affluenza${badgeHtml}</span>${crowdLabel}</div>` : '';
        card.innerHTML = `<div class="club-card-header"><div class="club-name">${escapeHtml(club.name || 'Locale')}</div>${showMapStars ? `<div class="club-stars">${clubStarsDisplayHtml(club)}</div>` : ''}</div>${metaLine}${tagsBlock}${crowdBlock}`;
        card.addEventListener('click', () => { setActiveCard(card); if (leafletMap) leafletMap.setView([club.lat, club.lng], Math.max(14, leafletMap.getZoom())); showClubDetails(club); });
...
function clubsForSidebar(clubs) { return clubs || []; }

function initMap() {
    const mapEl = document.getElementById('leafletMap'); if (!mapEl) return;
    leafletMap = L.map(mapEl, { zoomControl: true, preferCanvas: true }).setView([DEFAULT_CENTER.lat, DEFAULT_CENTER.lng], 6);
    L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', { attribution: '&copy; OpenStreetMap &copy; CARTO', maxZoom: 19 }).addTo(leafletMap);
    ...
    function requestUserLocation(preferGpsAsSearchCenter = false) {
        ...
        function applyPosition(pos, preferForCenter) {
            ...
            userLocation = here; discoveryCenter = null; discoveryRetryCount = 0; gpsRetryCount = 0;
            saveLastLocation(here);
            if (userLocationMarker) { userLocationMarker.remove(); userLocationMarker = null; }
            userLocationMarker = L.circleMarker([here.lat, here.lng], { radius: 6, weight: 2, color: '#22d3ee', fillColor: '#06b6d4', fillOpacity: 0.85 }).addTo(leafletMap).bindPopup('Sei qui');
            userLocationMarker.on('click', () => openInMapsChooser(here.lat, here.lng, 'La tua posizione'));
            if (!manualCenter && leafletMap) leafletMap.flyTo([here.lat, here.lng], 13, { duration: 0.75, easeLinearity: 0.25 });
            const useGpsAsCenter = preferGpsAsSearchCenter || !manualCenter;
            if (!useGpsAsCenter) return;
            if (preferGpsAsSearchCenter) {
                currentRadiusKm = 5;
                try { const rr = document.getElementById('radiusKmRange'); const rp = document.getElementById('radiusKmPill'); if (rr) rr.value = '5'; if (rp) rp.textContent = '5 km'; } catch { /* ignore */ }
            }
            manualCenter = null;
            if (tempSearchMarker) { tempSearchMarker.remove(); tempSearchMarker = null; }
            const skipClubReload = Boolean(prevLocForReload && !preferGpsAsSearchCenter && kmBetween(prevLocForReload, here) < 0.12);
            if (!skipClubReload) loadClubs();
        }
        const fastOpts = preferGpsAsSearchCenter ? { enableHighAccuracy: false, timeout: 4000, maximumAge: 300000 } : { enableHighAccuracy: false, timeout: 7000, maximumAge: 180000 };
        navigator.geolocation.getCurrentPosition((pos) => {
            applyPosition(pos, preferGpsAsSearchCenter);
            if (!preferGpsAsSearchCenter) return;
            navigator.geolocation.getCurrentPosition((pos2) => applyPosition(pos2, true), () => { /* ignore */ }, { enableHighAccuracy: true, timeout: 12000, maximumAge: 60000 });
        }, (err) => { console.warn('Geolocation non disponibile:', err && err.message ? err.message : err); loadClubs(); }, fastOpts);
    }
...
const SEED_VENUES = [
    { id: 'life-club-rovetta', name: 'Life Club Rovetta', lat: 45.8741, lng: 9.9717, address: 'Via Vogno 7, 24020 Rovetta (BG)' },
    { id: 'bar-da-spicchio-clusone', name: 'Bar Da Spicchio', lat: 45.8917564, lng: 9.9853276, address: 'Via Nuova 12, 24020 Rovetta (BG)', tipo: 'Locale', ingresso: 0 },
    { id: 'piccolo-bar-clusone', name: 'Piccolo Bar', lat: 45.8831737, lng: 9.9343768, address: 'Via Sales 2, 24023 Clusone (BG)', tipo: 'Locale', ingresso: 0 },
    { id: 'piccolo-bar-2-clusone', name: 'Piccolo Bar 2.0', lat: 45.8877922, lng: 9.9329784, address: 'Via Luigi Carrara 1, 24023 Clusone (BG)', tipo: 'Locale', ingresso: 0 }
];
...
function getBgPack() {
    try { const raw = localStorage.getItem(BG_PACK_KEY); if (!raw) return null; const o = JSON.parse(raw); if (!o || !Array.isArray(o.items) || !o.ts) return null; if (Date.now() - o.ts > BG_PACK_TTL_MS) return null; return o.items; } catch { return null; }
}

function setBgPack(items) {
    try { if (!Array.isArray(items) || !items.length) return; localStorage.setItem(BG_PACK_KEY, JSON.stringify({ ts: Date.now(), items })); } catch { /* ignore */ }
}

function isNearBergamo(center) {
    try { if (!center || !Number.isFinite(center.lat) || !Number.isFinite(center.lng)) return false; return kmBetween(center, BG_CENTER) <= 28; } catch { return false; }
}

async function prefetchBgPackIfNeeded(center) {
    if (!isNearBergamo(center)) return; if (getBgPack()) return;
    try {
        const clubs = await fetchClubsOverpassAnyEndpoint(BG_CENTER, 42000);
        if (!Array.isArray(clubs) || !clubs.length) return;
        const slim = clubs.slice(0, 140).map((c) => ({ id: `seedbg:${String(c.id || '')}`, name: c.name || 'Locale', address: c.address || '', lat: c.lat, lng: c.lng, ratingText: '—', starsText: '—', crowdText: 'Non impostata', crowdPercent: null, ageText: '—', source: 'Seed', osmAmenity: c.osmAmenity || '', osmClub: c.osmClub || '', osmLeisure: c.osmLeisure || '', isBar: Boolean(c.isBar) })).filter((x) => Number.isFinite(x.lat) && Number.isFinite(x.lng));
        setBgPack(slim);
    } catch { /* ignore */ }
}
...
function getLocaliPageOverpassCache(center, radiusMeters) {
    try { const key = localiPageOverpassCacheKey(center, radiusMeters); const raw = localStorage.getItem(key); if (!raw) return null; const cached = JSON.parse(raw); if (Date.now() - cached.ts > OVERPASS_CACHE_TTL_MS) { localStorage.removeItem(key); return null; } return cached.data; } catch { return null; }
}

function setLocaliPageOverpassCache(center, radiusMeters, data) {
    try { localStorage.setItem(localiPageOverpassCacheKey(center, radiusMeters), JSON.stringify({ ts: Date.now(), data })); } catch { /* quota */ }
}
...
async function fetchLocaliStyleOverpassElements(endpoint, center, radiusMeters) {
    ...
    const lat = Number(center.lat); const lng = Number(center.lng);
    const r = Math.max(1000, Math.min(50000, Math.round(radiusMeters)));
    const query = `[out:json][timeout:25];(node["amenity"="nightclub"](around:${r},${lat},${lng});way["amenity"="nightclub"](around:${r},${lat},${lng});node["amenity"="bar"](around:${r},${lat},${lng});way["amenity"="bar"](around:${r},${lat},${lng});node["amenity"="pub"](around:${r},${lat},${lng});way["amenity"="pub"](around:${r},${lat},${lng}););out center tags;`;
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 20000);
    let res;
    try { res = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }, body: new URLSearchParams({ data: query }), signal: controller.signal }); }
    catch (e) { const msg = e && e.name === 'AbortError' ? 'timeout' : (e && e.message ? e.message : String(e)); markOverpassEndpointFail(endpoint, msg); throw e; }
    finally { clearTimeout(timeout); }
    if (!res.ok) { if (res.status === 429) markOverpassEndpoint429(endpoint); else if (res.status >= 500 || res.status === 0) markOverpassEndpointFail(endpoint, `http_${res.status}`); throw new Error(`Overpass HTTP ${res.status}`); }
    const json = await res.json();
...
async function prefetchLocaliOverpassForCenter(center) {
    ...
    for (const ep of (eps.length ? eps : OVERPASS_ENDPOINTS)) {
        try { const els = await fetchLocaliStyleOverpassElements(ep, center, r); if (Array.isArray(els) && els.length) { setLocaliPageOverpassCache(center, r, els); return; } } catch { /* prova endpoint successivo */ }
    }
}
...
function fetchSeedPlacesWithinRadius(center, radiusKm) {
    const results = [];
    const rk = Number(radiusKm);
    const seedMaxKm = Number.isFinite(rk) ? Math.max(1, rk) : 1;
    if (isNearBergamo(center)) {
        const bg = getBgPack();
        if (Array.isArray(bg) && bg.length) {
            for (const seed of bg) { const d = kmBetween(center, { lat: seed.lat, lng: seed.lng }); if (d > seedMaxKm) continue; results.push({ ...seed, distanceKm: d, seedTipo: seed.isBar ? 'Locale' : (seed.seedTipo || null), seedIngresso: seed.seedIngresso ?? null }); }
        }
    }
    for (const seed of SEED_VENUES) {
        const d = kmBetween(center, { lat: seed.lat, lng: seed.lng });
        if (d > seedMaxKm) continue;
        results.push({ id: `seed:${seed.id}`, name: seed.name, address: seed.address || '', lat: seed.lat, lng: seed.lng, distanceKm: d, ratingText: '—', starsText: '—', crowdText: 'Non impostata', crowdPercent: null, ageText: '—', source: 'Seed', osmAmenity: seed.tipo ? '' : 'nightclub', osmClub: '', seedTipo: seed.tipo || null, seedIngresso: seed.ingresso ?? null });
    }
    return results;
}
...
async function loadClubs() {
    ...
    if (sidebar) {
        if (mobileMap) { sidebar.innerHTML = `<div class="map-sidebar-load-placeholder" aria-hidden="true"><span class="map-sidebar-load-dot"></span><span class="map-sidebar-load-dot"></span><span class="map-sidebar-load-dot"></span></div>`; showMapFetchLoading(hasActive); }
        else { hideMapFetchLoading(); sidebar.innerHTML = `<div class="club-card active" style="cursor:default;text-align:center;"><div class="club-name" style="font-size:0.85rem;">Caricamento locali…</div><div style="margin:0.8rem 0;"><div style="width:28px;height:28px;border:3px solid rgba(168,85,247,0.2);border-top-color:#a855f7;border-radius:50%;animation:spin .7s linear infinite;margin:0 auto;"></div></div><div class="club-meta">${hasActive ? 'Cerco i locali più vicini a te' : 'Ti mostro locali casuali in Italia'}</div></div>`; }
    }
    ...
    try {
        ...
        if (cachedInstant && cachedInstant.length && runId === loadClubsRunId) {
            clubsData = filterClubsWithinRadius(recomputeClubDistancesFrom(cachedInstant, listRef), currentRadiusKm).sort((a, b) => a.distanceKm - b.distanceKm).slice(0, MAP_MAX_VENUES_SHOWN);
            enrichClubsWithFirebase(clubsData); renderMarkers(clubsData); renderSidebar(clubsForSidebar(clubsData));
        }

        const seedInstant = fetchSeedPlacesWithinRadius(listRef, currentRadiusKm).sort((a, b) => a.distanceKm - b.distanceKm).slice(0, 18);
        if (seedInstant.length && !cachedInstant) { clubsData = seedInstant; enrichClubsWithFirebase(clubsData); renderMarkers(clubsData); renderSidebar(clubsForSidebar(clubsData)); }
        ...
        const applyOverpassClubsAndRender = async (rawClubs, radiusMetersUsed) => {
            ...
            for (const s of seed) {
                const exists = mergedClubs.some(c => normalizeText(c.name) === normalizeText(s.name) || kmBetween({ lat: c.lat, lng: c.lng }, { lat: s.lat, lng: s.lng }) < 0.15);
                if (!exists) mergedClubs.push(s);
            }
            mergedClubs = filterClubsWithinRadius(recomputeClubDistancesFrom(mergedClubs, listRef), currentRadiusKm).sort((a, b) => a.distanceKm - b.distanceKm).slice(0, MAP_MAX_VENUES_SHOWN);
            ...
            if (manualCenter && userLocation && dPlaceGpsMerge > SAME_AREA_GPS_VS_PLACE_KM && dPlaceGpsMerge < MERGE_GPS_SECOND_QUERY_MAX_KM) {
                try {
                    ...
                    if (nearMe && nearMe.length) mergedClubs = filterClubsWithinRadius(recomputeClubDistancesFrom(mergeClubListsById(mergedClubs, nearMe), listRef), currentRadiusKm).sort((a, b) => a.distanceKm - b.distanceKm).slice(0, MAP_MAX_VENUES_SHOWN);
                } catch { /* merge facoltativo */ }
            }
            const wantsAutoRatings = Boolean(GOOGLE_PLACES_API_KEY);
            clubsData = mergedClubs.map((c) => wantsAutoRatings ? { ...c, ratingsPending: true, ratingText: 'Recensioni in aggiornamento…', starsText: '…', source: c.source || 'OSM' } : { ...c, ratingsPending: false, ratingText: c.ratingText ?? '—', starsText: c.starsText ?? '—' });
            enrichClubsWithFirebase(clubsData); renderMarkers(clubsData); renderSidebar(clubsForSidebar(clubsData));
            loadAndRenderUserReviews(); loadAndRenderGoogleReviews();
            if (wantsAutoRatings) {
                enrichClubsWithGoogleRatings(clubsData).then((enriched) => {
                    if (runId !== loadClubsRunId) return;
                    clubsData = enriched; enrichClubsWithFirebase(clubsData); renderMarkers(clubsData); renderSidebar(clubsForSidebar(clubsData));
                    loadAndRenderUserReviews(); loadAndRenderGoogleReviews();
                }).catch((err) => {
                    console.warn('Arricchimento Google:', err);
                    if (runId !== loadClubsRunId) return;
                    clubsData = clubsData.map((c) => ({ ...c, ratingsPending: false, ratingText: (typeof c.ratingText === 'string' && c.ratingText.includes('aggiornamento')) ? '—' : (c.ratingText || '—'), starsText: (c.starsText === '…' || c.starsText == null) ? '—' : c.starsText }));
                    enrichClubsWithFirebase(clubsData); renderMarkers(clubsData); renderSidebar(clubsForSidebar(clubsData));
                });
            }
        };

        let clubs = null; let lastErr = null;
        ...
        if (clubs && clubs.length) {
            ...
            try { const googleClubs = await fetchNightclubsFromGooglePlaces({ center: listRef, radiusMeters }); if (googleClubs && googleClubs.length) clubs = googleClubs; } catch (e) { lastErr = e; }
            if (!clubs || !clubs.length) {
                if (isGpsMode && gpsRetryCount < 2) { gpsRetryCount++; currentRadiusKm = Math.max(currentRadiusKm, gpsRetryCount === 1 ? 25 : 40); return loadClubs(); }
                if (isDiscoveryMode && discoveryRetryCount < 2) { discoveryRetryCount++; discoveryCenter = pickDiscoveryCenter(); if (leafletMap && discoveryCenter) leafletMap.setView([discoveryCenter.lat, discoveryCenter.lng], 11); currentRadiusKm = Math.max(currentRadiusKm, 35); return loadClubs(); }
        ...
        if (!isDiscoveryMode && prefetchCenter && clubsData && clubsData.length) scheduleLocaliOverpassPrefetch(prefetchCenter);
        ...
        hideMapFetchLoading();
        const sidebar = document.querySelector('.map-sidebar');
        if (sidebar) sidebar.innerHTML = `<div class="club-card active" style="cursor:default;"><div class="club-card-header"><div class="club-name">Mappa non disponibile per un attimo</div><div class="club-stars">—</div></div><div class="club-meta">Controlla la connessione e aggiorna la pagina.</div><div class="club-tags"><span class="tag tag-hot">Riprova</span></div></div>`;
    }
}
    ...
    (async () => {
        try {
            if (window.isSecureContext && 'geolocation' in navigator && typeof window.__requestUserLocationAuto === 'function') {
                let geoDenied = false; let alreadyGranted = false;
                try { if (navigator.permissions?.query) { const st = await navigator.permissions.query({ name: 'geolocation' }); if (st.state === 'denied') geoDenied = true; if (st.state === 'granted') alreadyGranted = true; } } catch { /* Safari */ }
                if (!geoDenied && alreadyGranted) {
                    const warm = peekLastLocationForWarmStart();
                    if (warm && !manualCenter) { userLocation = warm; discoveryCenter = null; currentRadiusKm = Math.max(currentRadiusKm, 18); loadClubs(); }
                    window.__requestUserLocationAuto(); return;
                }
                if (!geoDenied) window.__requestUserLocationAuto();
            }
        } catch { /* ignore */ }
        if (!manualCenter && !userLocation) { discoveryCenter = pickDiscoveryCenter(); if (leafletMap && discoveryCenter) leafletMap.setView([discoveryCenter.lat, discoveryCenter.lng], 11); currentRadiusKm = Math.max(currentRadiusKm, 18); }
        else { currentRadiusKm = Math.max(currentRadiusKm, 18); }
        loadClubs();
    })();
    loadFirebaseVenues();
    loadAndRenderUserReviews();
...
(function __fmnLazyInitMap() {
    ...
    const io = new IntersectionObserver((entries) => {
        if (started) return;
        for (const e of entries) { if (!e.isIntersecting) continue; started = true; io.disconnect(); __fmnStartApp(); break; }
...
if (searchInput) {
    let placeMode = false;
    const onType = debounce(() => { if (placeMode) return; applySearchFilter(searchInput.value); }, 120);
    searchInput.addEventListener('input', () => { placeMode = false; onType(); });
    searchInput.addEventListener('keydown', async (e) => {
        ...
        try {
            ...
            if (place) {
                placeMode = true; applySearchFilter('');
        ...
        if (byName.length) { if (leafletMap) leafletMap.setView([byName[0].lat, byName[0].lng], 15); showClubDetails(byName[0]); }
    });
}
...
if (radiusRange) {
    ...
    radiusRange.addEventListener('input', () => { const v = Number(radiusRange.value); const km = Number.isFinite(v) ? Math.max(1, Math.min(30, Math.round(v))) : 12; if (radiusPill) radiusPill.textContent = `${km} km`; });
...
if (searchClear && searchInput) {
    searchClear.addEventListener('click', () => {
        ...
        if (tempSearchMarker) { tempSearchMarker.remove(); tempSearchMarker = null; }
        if (leafletMap) {
            if (userLocation) { leafletMap.setView([userLocation.lat, userLocation.lng], 13); persistMapFocusForLocali({ type: 'gps', lat: userLocation.lat, lng: userLocation.lng, label: 'La tua posizione' }); }
            else { try { localStorage.removeItem(FMN_MAP_FOCUS_KEY); } catch { /* ignore */ } leafletMap.setView([DEFAULT_CENTER.lat, DEFAULT_CENTER.lng], 13); }
        }
        loadClubs();
    });
}
...
if (useCurrentLocationBtn) {
    useCurrentLocationBtn.addEventListener('click', () => { if (typeof window.__requestUserLocation === 'function') window.__requestUserLocation(); });
}
...
if (directionsBtn) {
    directionsBtn.addEventListener('click', (e) => {
        ...
        const lat = Number(directionsBtn.dataset.lat); const lng = Number(directionsBtn.dataset.lng);
...
if (popupDiscoClose) popupDiscoClose.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); hideClubDetailsPopup(); });

(function () {
    const nav = document.getElementById('siteNav'); const btn = document.getElementById('navMenuBtn');
    const backdrop = document.getElementById('navMenuBackdrop'); const list = document.getElementById('navMainList');
    if (!nav || !btn || !backdrop || !list) return;
    function setNavOpen(open) { nav.classList.toggle('nav-open', open); btn.setAttribute('aria-expanded', open ? 'true' : 'false'); btn.setAttribute('aria-label', open ? 'Chiudi menu' : 'Apri menu'); document.body.style.overflow = open ? 'hidden' : ''; }
